...
x = 1:size(y,2);
x = x*binsize;
str = ['min = ',num2str(min(data),'%.2f'),'  max = ',num2str(max(data),'%.2f'),'  mean = ',num2str(mean(data)),'  median = ',num2str(median(data)),'    N = ',num2str(moleculeNums)];
switch fitOption
    case "gauss1"
        f = fit(x.',y.','gauss1');
        ...
        text(plotAxes,floor(f.b1),floor(f.a1)+5,num2str(f.b1,'%.2f'),'Color','r');
    ...
    case "poisson"
        ...
        str = ['y=exp(-',num2str(lambda,'%.3f'),'*',num2str(lambda,'%.3f'),'^x/x!',str];
        text(plotAxes,floor(max(x)/2),floor(max(y)/2),['lambda = ',num2str(lambda,'%.2f')],'Color','r');
end
